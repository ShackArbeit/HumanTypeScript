...
import { ReBookingInterface } from './ContextType';

export const ReDateTimeContext = createContext<ReBookingInterface|undefined>(undefined);
...
export default function ReDateTimeProvider({ children }:{
 ...
 const [secondItem,setSecondItem]=useState<string |null>('');
...
//  const [bookingIdToDelete, setBookingIdToDelete] = useState('');
 ...
 const handleSelectDateTime = (newDateTime:Date[]) => {
  ...
  try {
      ...
      const response = await fetch('http://localhost:8001/saveDateTimeAndItem', {
        method: 'POST',
        headers: {
    ...
    const userConfirmed = window.confirm('請確定要刪除預約嗎 ?');
     if (userConfirmed) {
      console.log('Deleting booking with ID:', bookingIdToDelete);
       const response = await fetch(`http://localhost:8001/deleteBooking/${bookingIdToDelete}`, {
         method: 'DELETE',
         headers: {
           'Content-Type': 'application/json',
         },
