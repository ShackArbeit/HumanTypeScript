...
import { DateTimeContextType } from './TypeOfContext';

export const DateTimeContext = createContext<DateTimeContextType|undefined>(undefined);
...
export default function DateTimeProvider({ children }: { children: ReactNode }) {
 ...
 const handleFirstAutocompleteChange = (event: React.ChangeEvent<{}>, newValue:string) => {
   setFirstValue(newValue);
   setSecondOptions(options[newValue] || []);
 };
 // 選取預約項目第二分項的 function 
 const handleSecondAutocompleteChange=(event:React.ChangeEvent<{}>,newItem:string)=>{
     setSecondItem(newItem)
     setSecondOptions(options[newItem] || []);
 }
 // 選取項目的 function 
  const handleSelectDateTime = (newDateTime:Date) => {
    setSelectDateTime((prevDates) => [...prevDates, newDateTime]);
  };
  ...
  const handleSendDateTime = async (event: MouseEvent) => {
    const currentDate = dayjs();
    dayjs.locale('zh-cn');
    // const selectedDate = dayjs(selectDateTime);
    const selectedDate = dayjs(selectDateTime[0]); 
